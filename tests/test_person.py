...
class PersonTestCase(CinemateTestCase):
    ...
    def test_person_movies(self):
        ...
        movies = self.cin.person(3971).movies()
        ...
        self.assertEqual(len(director_movies), 0)
        # Дописать здвесь тесты когда пофиксят
        # http://cinemate.cc/forum/topic/497/?page=9#post11885
        self.assertIn('actor', movies)
        actor_movies = movies['actor']
        self.assertIsInstance(actor_movies, list)
        self.assertEqual(len(actor_movies), 5)
        movie = actor_movies[0]
        self.assertIsInstance(actor_movies[0], self.cin.movie)
        self.assertIsInstance(movie, self.cin.movie)
        self.assertEqual(movie.id, 119304)
        self.assertEqual(movie.type, 'movie')
        self.assertEqual(movie.runtime, 107)
