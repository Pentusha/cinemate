...
class Cinemate(BaseCinemate):
    ...
    def api_get(self, url, fmt='json', passkey=False, apikey=False, **kwargs):
        """ Получить фтраницу api или сайта
        :param url: адрес получаемой страницы, полностью или p/a/th?param=1
        :type url: str
        :param fmt: json, xml или None
        :type fmt: types.NoneType, str
        ...
        if req.status_code != codes.ok:
            msg = 'cinemate return invalid status code {code}'.format(
